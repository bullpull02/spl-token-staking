...
impl User {
  ...
  pub fn update(&mut self, vault: &mut Account<Vault>) {
    ...
    let reward_earned: f64 = (daily_payout_amount as f64) * (staked_seconds as f64) / 86400f64 / (vault.total_staked_amount as f64) * (self.staked_amount as f64);
    self.earned_amount = self.earned_amount.checked_add(reward_earned as u64).unwrap();
  }
}